...
import { updateMeal, getMealById, deleteMeal } from "../../../actions";
  ...
  useEffect(() => {
    const loadMeal = async () => {
      ...
      if (result.success && result.data) {
        ...
        setTime(meal.time?.toString() || "");
        setServingsize(meal.servingsize || "");
        setImage(meal.image || "");
        setType(meal.type || "");
        setSteps(meal.steps && meal.steps.length > 0 ? meal.steps : [""]);
        setIngredients(meal.ingredients || []);
        setCalories(meal.calories?.toString() || "");
        setCarbohydrates(meal.carbohydrates?.toString() || "");
        setFat(meal.fat?.toString() || "");
        setProtein(meal.protein?.toString() || "");
        setSodium(meal.sodium?.toString() || "");
        setSugar(meal.sugar?.toString() || "");
        setEnergy(meal.energy?.toString() || "");
      } else {
        setError(result.error || "Rezept nicht gefunden");
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || "Fehler beim Aktualisieren des Rezepts");
        setSaving(false);
      }
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || "Fehler beim Löschen des Rezepts");
        setDeleting(false);
      }
    } catch (err) {
      setError("Ein unerwarteter Fehler ist aufgetreten");
    ...
    return (
      <div className="max-w-4xl mx-auto">
        <div className="text-center py-12">
          <p className="text-slate-600">Lade Rezept...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-slate-600">
          Aktualisiere die Informationen für dieses Rezept
        </p>
      </div>
